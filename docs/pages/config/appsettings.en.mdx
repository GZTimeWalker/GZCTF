...
- **Prometheus**: Prometheus endpoint support.
  ...
  - **Port**: Optional. Set the port that Prometheus endpoint listens on. If this is configured, additional manual configuration of `Kestrel.Endpoints` is required to make the server listen to the specified port normally.

...
これをGZCTFをデプロイするマシンに保存し、`docker-compose.yml`と同じフォルダに置きます。例えば`kube-config.yaml`とします。
次に`docker-compose.yml`のマウント情報を変更します：
...
もし、一台のk3sインスタンス上で255個以上のPodを実行する必要がある場合、デフォルトのサブネットサイズを超える可能性があります。その場合、k3sをインストールする際に`INSTALL_K3S_EXEC`を指定し、`node-cidr-mask-size`を必要なサブネットサイズに変更します。

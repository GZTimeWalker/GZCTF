...
GZCTF は Docker イメージを通じて提供され、docker-compose を通じて完全に機能する CTF プラットフォームを迅速にデプロイできます。 これが唯一のデプロイ方法ではないことに注意してください。Kubernetes の使用をお勧めします。 関連するデプロイメント方法については、[デプロイメント](/deployment) を参照してください。
...
1. 構成を円滑に進められるために、次の初期化パラメータを準備してください。
   ...
   - `GZCTF_ADMIN_PASSWORD`：初期管理者パスワード。 データベースが初期化されていない状態で有効となり、初回起動時に設定する必要があり、`docker-compose.yml` で設定する必要があります。
   - `POSTGRES_PASSWORD`: データベースのパスワード。`docker-compose.yml` と `appsettings.json` で設定する必要があります。
...
3. 次のコンテンツを `docker-compose.yml` ファイルとして保存し、**初期化パラメーターに置き換えってください**。
   ...
   ネットワーク セグメントの競合が発生する可能性があります。その場合は `docker-compose.yml` のサブネット構成
...
Docker をコンテナ バックエンドとして使用する場合は、GZCTF が指定されたポートをホスト上のランダム ポートに自動的にマップするため、質問 Dockerfile で `EXPOSE` を使用してポートを公開しないでください。複数のポートがポート リソースを占有し、セキュリティ問題が発生する恐れがあります。

Kubernetes をコンテナ バックエンドとして使用する場合、Kubernetes の GZCTF が特定のポートのサービスを作成するため、この構成は影響を受けません。

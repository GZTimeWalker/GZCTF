...
1. To prepare for the following configuration, please prepare the following initialization parameters:
   ...
   - `GZCTF_ADMIN_PASSWORD`: Initial administrator password, effective when the database is not initialized, needs to be set at the first startup. It should be configured in `docker-compose.yml`.
   - `POSTGRES_PASSWORD`: Database password, needs to be configured in `docker-compose.yml` and `appsettings.json`.
...
3. Save the following content as the `docker-compose.yml` file, **and replace with your initialization parameters**.
   ...
   You might encounter network segment conflicts, please refer to `docker-compose.yml` subnet configuration or `/etc/docker/daemon.json` subnet configuration.
...
If you use Docker as the container backend, please do not use `EXPOSE` to expose ports in the challenge Dockerfile, because GZCTF will automatically map the specified port to a random port on the host. If you use `EXPOSE` in the Dockerfile, it will cause a challenge to expose multiple ports, occupy port resources, and leave security risks.

If you use Kubernetes as the container backend, this configuration is not affected, because in Kubernetes, GZCTF will create a Service for specific ports.

...
public class OpenApiTests
{
    ...
    public OpenApiTests(GZCTFApplicationFactory factory, ITestOutputHelper output)
    {
        _output = output;
    ...
    public async Task OpenApi_Spec_IsValidJson()
    {
        ...
        _output.WriteLine($"OpenAPI version: {openApiVersion.GetString()}");
        _output.WriteLine($"Title: {info.GetProperty("title").GetString()}");
        _output.WriteLine($"Number of paths: {paths.EnumerateObject().Count()}");
    }
    ...
    public async Task OpenApi_ContainsExpectedEndpoints()
    {
        ...
        foreach (var endpoint in expectedEndpoints)
        {
            ...
            _output.WriteLine($"Endpoint '{endpoint}': {(hasEndpoint ? "Found" : "Missing")}");
    ...
    public async Task OpenApi_HasSchemaDefinitions()
    {
        ...
        _output.WriteLine($"Number of schema definitions: {schemaCount}");
        ...
        _output.WriteLine($"Sample schemas: {string.Join(", ", schemaNames)}");
    }

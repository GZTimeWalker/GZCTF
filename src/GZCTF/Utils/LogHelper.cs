...
public static class LogHelper
{
    ...
    extension<T>(ILogger<T> logger)
    {
        ...
        public void LogErrorMessage(Exception exception, string msg) =>
        ...
        public void Log(string msg, UserInfo? user, TaskStatus status,
            LogLevel? level = null) =>
            Log(logger, msg, user?.UserName ?? "Anonymous", user?.IP?.ToString(), status, level, null);
        ...
        public void Log(string msg, HttpContext? context, TaskStatus status,
            LogLevel? level = null)
        {
            var username = context?.User.Identity?.Name ?? "Anonymous";
            var fingerprint = ContextHelper.GetValidBrowserFingerprint(context?.User);

            Log(logger, msg, username, context?.Connection.RemoteIpAddress?.ToString(), status, level, fingerprint);
        }
        ...
        public void Log(string msg, IPAddress? ip, TaskStatus status,
            LogLevel? level = null)
            => Log(logger, msg, "Anonymous", ip?.ToString(), status, level, null);
        ...
        public void Log(string msg, string uname, string? ip, TaskStatus status,
    ...
    extension(WebApplication app)
    {
        ...
        {
            var enableQueryLogging =
                app.Environment.IsDevelopment() && app.Configuration.GetValue("QUERY_LOGGING", false);
            ...
            {
                var options = app.Services.GetRequiredService<IOptions<OpenTelemetry.Exporter.OtlpExporterOptions>>()
                    .Value;
                loggerConfig = loggerConfig.WriteTo.OpenTelemetry(options.Endpoint.ToString(), options.Protocol switch
                {
                    OpenTelemetry.Exporter.OtlpExportProtocol.HttpProtobuf => Serilog.Sinks.OpenTelemetry.OtlpProtocol
                        .HttpProtobuf,
    ...
    {
        public string RenderMessageWithExceptions()
        {

...
public class SwarmManager : IContainerManager
{
    ...
    public SwarmManager(IContainerProvider<DockerClient, DockerMetadata> provider, ILogger<SwarmManager> logger, IStringLocalizer<Program> localizer)
    {
        ...
        logger.SystemLog(Program.LocalizerForLogging[nameof(Resources.Program.ContainerManager_SwarmMode)], TaskStatus.Success, LogLevel.Debug);
    }
    ...
    CreateContainer:
        ...
        catch (DockerApiException e)
        {
            ...
            {
                _logger.SystemLog(Program.LocalizerForLogging[nameof(Resources.Program.ContainerManager_ContainerExisted), parameters.Service.Name], TaskStatus.Duplicate,
                    LogLevel.Warning);
            ...
            _logger.SystemLog(Program.LocalizerForLogging[nameof(Resources.Program.ContainerManager_ContainerCreationFailedStatus), parameters.Service.Name, e.StatusCode], TaskStatus.Failed, LogLevel.Warning);
            _logger.SystemLog(Program.LocalizerForLogging[nameof(Resources.Program.ContainerManager_ContainerCreationFailedResponse), parameters.Service.Name, e.ResponseBody], TaskStatus.Failed, LogLevel.Error);
            return null;
        }
        catch (Exception e)
        {
            _logger.LogError(e, Program.LocalizerForLogging[nameof(Resources.Program.ContainerManager_ContainerDeletionFailed), parameters.Service.Name]);
            return null;
        }
        ...
        do
        {
            ...
            if (retry == 3)
            {
                _logger.SystemLog(Program.LocalizerForLogging[nameof(Resources.Program.ContainerManager_ContainerPortNotExposed), container.ContainerId], TaskStatus.Failed,
                    LogLevel.Warning);
                return null;
            }
